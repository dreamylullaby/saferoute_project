...
class PerfilPage extends StatefulWidget {
  const PerfilPage({super.key, this.datasource, this.onLogout});
...
class _PerfilPageState extends State<PerfilPage> {
  ...
  Future<void> _verificarUbicacion() async {
    ...
    if (mounted) {
      setState(() {
        _ubicacionActiva = permiso == LocationPermission.always ||
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Esta acción es irreversible. Tu cuenta será eliminada y tus datos personales anonimizados.',
              style: GoogleFonts.inter(fontSize: 14, color: textM, height: 1.5),
            ),
            const SizedBox(height: 8),
            Text(
              'Tus reportes se mantendrán de forma anónima en el sistema.',
        ...
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: Text('Cancelar', style: GoogleFonts.inter(color: textS)),
          ),
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
  ...
  Widget _actionTile(
    ...
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final iconColor =
        danger ? AppColors.error : (enabled ? AppColors.primary : mutedColor);
